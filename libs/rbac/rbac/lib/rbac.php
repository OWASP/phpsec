...
abstract class BaseRBAC extends JModel
{
	...
	function Count()
	{
		$Res = \phpsec\SQL ( "SELECT COUNT(*) FROM {$this->TablePrefix()}{$this->Type()}" );
	...
	function PathID($Path)
	{
		...
		$res = \phpsec\SQL ( "SELECT node.ID,{$GroupConcat} AS Path
		...
		if ($res)
			...
			return null;
			// TODO: make the below \phpsec\SQL work, so that 1024 limit is over
		...
		print_ ( $PartsRev );
		$res = call_user_func_array ( "\phpsec\SQL", $PartsRev );
		if ($res)
			return $res [0] ['ID'];
		else
			return null;
	...
	function Reset($Ensure = false)
	{
		...
		$res = \phpsec\SQL ( "DELETE FROM {$this->TablePrefix()}{$this->Type()}" );
		$Adapter = get_class(jf::$DB);
		if ($this->IsMySql())
			\phpsec\SQL ( "ALTER TABLE {$this->TablePrefix()}{$this->Type()} AUTO_INCREMENT=1 " );
		elseif ($this->IsSQLite())
			\phpsec\SQL ( "delete from sqlite_sequence where name=? ", $this->TablePrefix () . "{$this->Type()}" );
		else
			throw new \Exception ( "RBAC can not reset table on this type of database: {$Adapter}" );
		$iid = \phpsec\SQL ( "INSERT INTO {$this->TablePrefix()}{$this->Type()} (Title,Description,Lft,Rght) VALUES (?,?,?,?)", "root", "root",0,1 );
		return (int)$res;
	}
	...
	{
		return \phpsec\SQL ( "INSERT INTO {$this->TablePrefix()}rolepermissions
	...
	{
		return \phpsec\SQL ( "DELETE FROM {$this->TablePrefix()}rolepermissions WHERE
	...
	function ResetAssignments($Ensure = false)
	{
		...
		$res = \phpsec\SQL ( "DELETE FROM {$this->TablePrefix()}rolepermissions" );
		
		$Adapter = get_class(jf::$DB);
		if ($this->IsMySql())
			\phpsec\SQL ( "ALTER TABLE {$this->TablePrefix()}rolepermissions AUTO_INCREMENT =1 " );
		elseif ($this->IsSQLite())
			\phpsec\SQL ( "delete from sqlite_sequence where name=? ", $this->TablePrefix () . "_rolepermissions" );
		else
			throw new \Exception ( "RBAC can not reset table on this type of database: {$Adapter}" );
...
class PermissionManager extends BaseRBAC
{
	...
	{
		$res = \phpsec\SQL ( "DELETE FROM {$this->TablePrefix()}rolepermissions WHERE
			PermissionID=?", $ID );
		return (int)$res;
	}
	...
	function Roles($Permission, $OnlyIDs = true)
	{
		...
		if ($OnlyIDs)
		{
			$Res = \phpsec\SQL ( "SELECT RoleID AS `ID` FROM
			...
			return \phpsec\SQL ( "SELECT `TP`.* FROM {$this->TablePrefix()}rolepermissions AS `TR`
...
class RoleManager extends BaseRBAC
{
	...
	{
		$r = \phpsec\SQL ( "DELETE FROM {$this->TablePrefix()}rolepermissions WHERE
			RoleID=? ", $ID );
		return $r;
	}
	/**
	...
	{
		return \phpsec\SQL ( "DELETE FROM {$this->TablePrefix()}userroles WHERE
			RoleID=?", $ID );
	}
	...
	function Permissions($Role, $OnlyIDs = true)
	{
		if ($OnlyIDs)
		{
			$Res = \phpsec\SQL ( "SELECT PermissionID AS `ID` FROM {$this->TablePrefix()}rolepermissions WHERE RoleID=? ORDER BY PermissionID", $Role );
			if (is_array ( $Res ))
			{
				$out = array ();
				foreach ( $Res as $R )
...
class RBACUserManager extends JModel 
{
	...
	function HasRole($Role, $User = null)
	{
		...
		if ($User === null)
			$User = jf::CurrentUser ();
		$R = \phpsec\SQL ( "SELECT * FROM {$this->TablePrefix()}userroles AS TUR
	...
	function Assign($Role, $UserID = null)
	{
		...
		$res = \phpsec\SQL ( "INSERT INTO {$this->TablePrefix()}userroles
	...
	function Unassign($Role, $UserID = null)
	{
		if ($UserID === null)
			$UserID = jf::CurrentUser ();
		return \phpsec\SQL ( "DELETE FROM {$this->TablePrefix()}userroles
	...
	function RoleCount($UserID = null)
	{
		if ($UserID === null)
			$UserID = jf::CurrentUser ();
		$Res = \phpsec\SQL ( "SELECT COUNT(*) AS Result FROM {$this->TablePrefix()}userroles WHERE UserID=?", $UserID );
	...
	function ResetAssignments($Ensure = false)
	{
		...
		$res = \phpsec\SQL ( "DELETE FROM {$this->TablePrefix()}userroles" );
		
		$Adapter = get_class(jf::$DB);
		if ($this->IsMySql())
			\phpsec\SQL ( "ALTER TABLE {$this->TablePrefix()}userroles AUTO_INCREMENT =1 " );
		elseif ($this->IsSQLite())
			\phpsec\SQL ( "delete from sqlite_sequence where name=? ", $this->TablePrefix () . "_userroles" );
		else
			throw new \Exception ( "RBAC can not reset table on this type of database: {$Adapter}" );
...
class RBACManager extends JModel
{
	...
	function Check($Permission, $UserID )
	{
		...
		$Res=\phpsec\SQL ( "SELECT COUNT(*) AS Result
 							FROM
